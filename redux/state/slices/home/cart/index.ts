...
const addCartApi = (payload: any) =>
  createAsyncThunk("post/myCart", async (payload1) => {
...
const cartQuantityApi = (payload: any) =>
...
const saveProductApi = (payload: any) =>
  createAsyncThunk("post/myCart", async (payload1) => {
    const { data } = await martApi
...
const deleteBulkSavedApi = (payload: any) =>
  createAsyncThunk("post/cart-quantity", async (payload1) => {
    const { data } = await martApi
      .post(`/user/saved-items/delete-bulk`, payload, await jsonHeader(""))
      .then((e) => e)
      .catch((e) => e.response);
    return data;
  });
